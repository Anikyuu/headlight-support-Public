import {palette} from './model.js?v=20260906-centered-device';
import {pixels} from './pixels.js';
import {mangaEye,bodyArtwork,mouthArtwork} from './artwork.js';

// Shared by the live design studio and the generated homepage gallery.
const escape=value=>String(value).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
export function createPhoneArtwork(state, {date=new Date(), text=key=>key, appText=key=>key, transparentBackground=false,paletteOverride,showDeviceIndicators=true}={}) {
const iconPaths={
...
function renderPhone(closed=false){
 const p=paletteOverride||palette(state),illustrated=state.style==='illustration',retro=state.retro;
 const now=date,day=now.getDate(),month=now.toLocaleDateString('en',{month:'short'}).toUpperCase(),weekday=now.toLocaleDateString('en',{weekday:'short'});
