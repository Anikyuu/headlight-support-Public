...
## Voice-entry illustration

The first introduction now includes a silent, 9.5-second illustration between its heading and paragraph: an outlined hand taps the large microphone once, a small waveform accompanies the spoken example, the send command appears, then the words become a paper note. The example is intentionally short (“牛乳を買う。” / “Buy milk.”) for mobile readability. The controls and example are localized in eight languages with the app’s actual send words. This is an illustrative explanation, not a recording control or an exact app screenshot.

`voice-entry-demo.js` draws all animated properties from one elapsed-time value. It plays once when at least 30% of the figure is in view, freezes outside the viewport or in a hidden tab, and ends on the saved note. Pause/resume and replay are available. Reduced Motion and the no-JavaScript fallback show the final static illustration; the reduced-motion version hides playback controls. No audio, microphone permission, remote assets, new dependency, or continuous post-completion timer is used. The existing website was edited directly; no video-render project was introduced.

Verified in-browser: tap hand, intermediate transcript and waveform, explicit spoken send, final saved note, pause stability, resume, replay, 390px and 1280px layouts, and all eight localized labels/examples without horizontal overflow. Reduced-motion handling is implemented but was not separately toggled in browser QA.
