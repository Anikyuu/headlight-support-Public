...
Kai clarified that the section must first explain what Head-Light is, and requested as few words as possible without repeating the same phrases. The heading now combines the comparison and identity: “どんなメモアプリよりも、気軽で楽しい手帳へ。” There is no separate introduction repeating that line. The functional text is shortened to “マイクを一度押し、話して「送信」で保存。文字でも入力できます。” These lines are localized in eight languages. The comparison is aspirational; no competitor benchmark was conducted.
